/*!
 * script.js - script interpreter for ldogejs
 ...
 * @property {Array<Opcode>} code - Parsed script code.
...
class Script {
  ...
  hasCanonicalPushes() {
    ...
    for (const op of this.code) {
      if (op.value > opcodes.OP_16)
        continue;
      if (op.value < opcodes.OP_PUSHDATA1 && op.value > opcodes.OP_0 && (op.toLength() === 1 && op.data[0] <= 16))
        // Could have used an OP_n code, rather than a 1-byte push.
        return false;
      if (op.value === opcodes.OP_PUSHDATA1 && data.size() < opcodes.OP_PUSHDATA1)
        // Could have used a normal n-byte push, rather than OP_PUSHDATA1.
        return false;
      if (op.value === opcodes.OP_PUSHDATA2 && data.size() <= 0xFF)
        // Could have used an OP_PUSHDATA1.
        return false;
      if (op.value === opcodes.OP_PUSHDATA4 && data.size() <= 0xFFFF)
